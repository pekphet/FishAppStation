...
class DataManager {
    object AppInfoDM {
        ...
        fun trans(data: RespAppListInfo) = data.run {
            AppInfo(appId, packageName, appName, iconUrl, downloadCount, Framework.Trans.Size(size), size, tips, downloadUrl, if (appSource == 1) 0 else versionCode, UrlCalls(imprUrl
    ...
    object Transor {
        fun CommentTransor(data: RespCommentList) = data.comments.node.map { Comment(it.authorImg, 0, it.authorName, Framework.Date.toYMD(it.date), it.content, it.thumbsupCount, it.thumbsupSign, it.commentId) }.toTypedArray()

        fun BannerTransor(data: RespBanners) = data.banners.banner.map { Banner(it.image, it.link) }
...
//        fun urlCallsTrans(data: RespAppListCallUrls?) = if (data == null) null else UrlCalls(data.imprUrl, data.downloadStartUrl, data.downloadFinishUrl, data.installFinishUrl)
    ...
    data class AppInfo(val appId: Int, val pkg: String, val name: String, val icon: String, val installCnt: Long, val size: String, val sizeInt: Int, val tip: String, val dlUrl: String, val vCode: Int, val calls: UrlCalls?, val source: Int)
