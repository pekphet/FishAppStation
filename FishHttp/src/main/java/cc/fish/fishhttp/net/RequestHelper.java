...
import cc.fish.fishhttp.util.Loggable;
 ...
 */
public class RequestHelper<T> implements Loggable{
    ...
    public static final String REQ_SUCCESS_WITH_NULL_DATA = "200,NULL DATA";
    ...
    public RequestHelper PostParams(Map<String, String> data) {
        if (isContentTypeJson) {
            isContentTypeJson = false;
        }
        boolean isFirstParam = true;
    ...
    public RequestHelper PostJson(Object obj) {
        return postJsonStr(new Gson().toJson(obj));
    }

    public RequestHelper postJsonStr(String json) {
        isContentTypeJson = true;
        mPostParam = new StringBuilder();
        mPostParam.append(json);
        return this;
    }
...
//        ZLog.e("JSON GOT", responseContent);
        T responseObj = null;
        try {
            if (mResultClz != null && mResultClz != String.class) {
                responseObj = new Gson().fromJson(responseContent, mResultClz);
            } else if (mResultClz == String.class || mTypeToken.getRawType() == String.class) {
                responseObj = (T) responseContent;
            } else if (mTypeToken != null) {
                responseObj = new Gson().fromJson(responseContent, mTypeToken.getType());
            }
        } catch (Exception e) {
            e.printStackTrace();
            doFailed(h, mUrl + ":PLZ make sure using correct result type/class");
        }
        if (responseObj == null) {
            doFailed(h, REQ_SUCCESS_WITH_NULL_DATA);
        } else {
...
//    }
    ...
    public RequestHelper EncryptSortedParam() {
        ...
        if (mPostParam != null && mPostParam.length() > 0) {
            ...
            for (String postP : postPs.split("&")) {
                try {
                    params.put(postP.split("=")[0], URLEncoder.encode(postP.replaceFirst(postP.split("=")[0] + "=", ""), "utf-8"));
                } catch (Exception ex) {
                    ex.printStackTrace();
        ...
        this.UrlParam("sign", MD5Utils.md5Encrypt(MD5Utils.sortParams(params)), !mUrlParam.toString().contains("?"));
        return this;
    }

    /****ADD HTTPS SUPPORT****/
    public RequestHelper HTTPS(Context context, String pemFile, String p12File) {
        InputStream keyIS = null;
        InputStream pemIS = null;
        try {
            KeyStore keyStore = KeyStore.getInstance("PKCS12");
            keyIS =context.getAssets().open(p12File);
    ...
    public String toLog() {
        ...
        sb.append(mUrl).append(mUrlParam).append("\n");
        if(mPostParam.length() > 0) {
            sb.append("FORM:").append(mPostParam).append("\n");
        }
        return sb.toString();
    }
